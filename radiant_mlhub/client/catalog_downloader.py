...
class CatalogDownloaderConfig(BaseModel):
    ...
    intersects: Optional[Union[Tuple[float], List[float], GeoJSON]] = None
...
class CatalogDownloader():
    ...
    def __init__(self, config: CatalogDownloaderConfig):
        ...
        if config.intersects: #need to clarify it was the GeoJSON passed in
            # & type is GeoJSON
            intersect_type = isinstance(config.intersects, )
            if type(config.intersects) == 
